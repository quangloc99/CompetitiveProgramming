 ...
 * Editorial: https://codeforces.com/blog/entry/57796
 * The main part is that we must have some datastructure to maintain a lower convex hull of a set of line.
 * My implementation is base on this one: https://github.com/niklasb/contest-algos/blob/master/convex_hull/dynamic.cpp
 * The idea is to dfs from node 1. When we are at node u, dfs all its children, then we got all the set of hull of its children.
 * Using something like DSU, merge small into big, we are able to solve this one in O(nlogn *logn) = O(n log^2 n)
 */
...
struct lineContainer : multiset<line> {  // maintains lower hull
