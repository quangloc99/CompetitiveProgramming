 ...
 * The idea can be found here: https://codeforces.com/blog/entry/109, but it is not really detailed.
 * So here are some more observation: S(1, 2) (or S(2, 3); S(3, 1)) can only be a line and circle (you can use geogebra to check ;)).
 * So there are totally 3 cases that need to handle: line-line intersection, circle-circle intersection and line-circle intersection.
 * But coding all of them may be too long. In order to get less code, I handle the line-circle intersection case as follow:
 * First I reflect the circle with reflect axis is the line, so now we got 2 circle, and the intersection of these circles
 * is our answer.
 * Also for less branching, I use OOP style.
 */
#include <bits/stdc++.h>
using namespace std;
