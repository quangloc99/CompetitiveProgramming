 ...
 * Editorial: https://codeforces.com/blog/entry/57223
 * Idea:
 *   It is clear that we need to use segment tree + euler tour technique to do the second and the third query.
 *   I used the 3rd type of euler tour (see here https://codeforces.com/blog/entry/18369), because it support rerooted query.
 *   So, for the first query, I just assign it to the current root, that all
 *   For the second and the third one, there are 2 subproblems:
 *   - Finding lca: this is simple, if we are finding lca of nodes u and v, and the current root is x, we can do like the following:
 *     First, find the up array (or whatever array you like to call it) to find lca with binary lifting with root 1.
 *     Let's call that function lca1. So the answer is the deepest node (again, from root 1) of lca1(u, x), lca(v, x) and lca(u, v).
 *   - The second is to find the range to update/query on segment tree.
 *     First, for each node u, I store all position of u on euler tour array to etPos[u]. Let's etRootPos be the current root' position on
 *     euler tour array. Then when we want to find the range of the subtree u (the first and the last node), we just binary search
 *     etRootPos on etPos[u] to find 2 closest nodes. There are edge cases, so see the function findRanges.
 */
#include <bits/stdc++.h>
using namespace std;
