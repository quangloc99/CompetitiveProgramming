 ...
 * Idea:
 * first, draw the Venn diagram. We got 7 region (we don't have the case of "empty set"). Using some math and
 * inclusion-exclusion, I count each number in each region separely (in the code, it is the array `cntDivsSeparete`.
 * After that, we can do brutefore-ly: we distribute the side into each region (that region must contain the side).
 * for each distribution, we can count like the following: 
 * - For each region, let's `value` be the number division in its, and `case` be the number of side that distributed into it.
 * - If case == 0, then the answer for this region is 0
 * - If case == 1, then answer = value
 * - Case == 2, answer = 1 + 2 + 3 + .. + value   // the the code for the shorten formula
 * - case == 3, answer = 1^2 + 2^2 + 3^2 + ... + value^2  // also see the code.
 * The final result is the product of the answer of all regions. Also each distribution must count as 1, see the part the process
 * `cases`.
 *
 * Note: there are a lot of preprocessions to reduce the time complexity
 */
import java.io.*
import java.util.*
