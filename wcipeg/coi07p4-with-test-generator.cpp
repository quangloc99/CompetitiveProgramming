...
unsigned llong safe_mul(unsigned llong x, int t, unsigned llong bound = LLONG_MAX) {
    if (bound / t < x) return bound;
    return x * t;
}
...
unsigned llong count_le(unsigned llong a) {
    ...
    for (n2 = 0, s2 =  1; s2 <= a and n2 < max2; s2 = safe_mul(s2, 2), ++n2) 
    for (n3 = 0, s3 = s2; s3 <= a and n3 < max3; s3 = safe_mul(s3, 3), ++n3) 
    for (n5 = 0, s5 = s3; s5 <= a and n5 < max5; s5 = safe_mul(s5, 5), ++n5) 
    for (n7 = 0, s7 = s5; s7 <= a and n7 < max7; s7 = safe_mul(s7, 7), ++n7) {
        auto s = to_str(a / s7);
        ...
        // if (loc_ans) 
            // clog << s7 << ' ' << s << ';' << n2 << ' ' << n3 << ' ' << n5 << ' ' << n7 << '=' << loc_ans << endl;
        loc_ans += count_le(s, pad0, false, {n2, n3, n5, n7});
        ans += loc_ans;
        // if (loc_ans) 
            // clog << s7 << ' ' << s << ';' << n2 << ' ' << n3 << ' ' << n5 << ' ' << n7 << '=' << loc_ans << endl;
    }
    // clog << a << ' ' << ans << endl << endl;
    return ans;
}

unsigned llong solve(unsigned llong a, unsigned llong b) {
    memset(lower_calculated, 0, sizeof(lower_calculated));
    llong res = count_le(b) - count_le(a - 1);
    return res;
}

unsigned llong digit_product(unsigned llong a) {
    llong res= 1;
    for (; a > 0; a /= 10) res *= a % 10;
    return res;
}

unsigned llong brute_solve(unsigned llong a, unsigned llong b) {
    clog << "Brute" << endl;
    unsigned llong ans = 0;
    map<unsigned llong, vector<llong>> cnt;
    for (unsigned llong i = 1; i <= b; ++i) {
        unsigned llong s = digit_product(i) * i;
        if (s >= a and s <= b) {
            ++ans;
            clog << i << ' ' << s << ' ' << digit_product(i) << endl;
            cnt[digit_product(i)].push_back(i);
        }
    }
    clog << "cnt" << endl;
    for (auto i: cnt) clog << i.xx << ' ' << len(i.yy) << endl;
    return ans;
}
...
int main(void) {
    ...
    if (ntest) {
        while (ntest--) {
            gen_test();
            unsigned llong u = solve(a, b);
            cout << u;
            // llong v = brute_solve(a, b);
            // cout << u << ' ' << v << endl;
            // assert(u == v);
        }
    } else {
